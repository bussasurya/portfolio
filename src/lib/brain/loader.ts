...
import { IContentDocument, IContentSection, IKnowledgeMetadata } from './types';
 ...
 */
export function parseFrontmatter(rawContent: string): { metadata: Record<string, any>; body: string } {
  const separator = '---';
 ...
 */
export function parseSections(markdown: string): IContentSection[] {
 ...
 */
export function validateMetadata(metadata: Record<string, any>, filePath: string): void {
...
export class ContentLoader {
  private cache: IContentDocument[] | null = null;
   ...
   */
  public async loadAll(forceReload: boolean = false): Promise<IContentDocument[]> {
    ...
    for (const filePath of files) {
      try {
        ...
        const document: IContentDocument = {
          ...
          rawContent: body,
          metadata: metadata as IKnowledgeMetadata,
          sections,
          hash
        };
        ...
        console.error(`[ContentLoader] Failed parsing document: ${filePath}. Error: ${err?.message}`);
